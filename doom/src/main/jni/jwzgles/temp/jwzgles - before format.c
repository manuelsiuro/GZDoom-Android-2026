...
void
jwzgles_glTexCoord4fv_TEST (const GLfloat *v)
{
...
void
jwzgles_glColor4fv_TEST (const GLfloat *v)
{
      if (state->compiling_verts)	/* inside glBegin */
        {
...
void
jwzgles_glVertex3f_TEST (GLfloat x, GLfloat y, GLfloat z)
{
  GLfloat v[4];
  v[0] = x;
  v[1] = y;
...
enable_disable (GLuint bit, int set)
{
   ...
   */
    if (flag)
    {
      unsigned long *enabled = &state->enabled;
      if (set > 0)
        *enabled |= flag;
      else if (set < 0)
        *enabled &= ~flag;
