...
#ifdef OPENTOUCH_SDL_EXTRA
    /* Rotation recreates only the surface (the context survives); the touch
       glue re-inits its aspect-fit framebuffer off this flag, so raise it for
       new surfaces as well as new contexts. */
    newEglCreated = 1;
#endif
