...
## The native engine (GZDoom 4.15, emileb's mobile_4.15.x)

The native layer is **rebased onto emileb's maintained GZDoom 4.15 mobile port** ([`github.com/emileb/gzdoom` @ `mobile_4.15.x`](https://github.com/emileb/gzdoom), the Delta Touch engine) plus his OpenTouch support libraries. Everything is **vendored** under `doom/src/main/jni/` (no submodules):
...
- `Game` — the gameplay activity, now a thin subclass of `org.libsdl.app2012.SDLActivity`: overrides `getLibraries()` (`hidapi, saffal, openal, zmusic_g4d, touchcontrols, SDL2, gzdoom`) and kills the process on destroy (engine can't re-init in-process; the activity runs in the separate `:Game` process). Launch contract from the UI is unchanged: Intent extras `args`, `game_path`, `res_div`, `game`.
...
- The app shell is modern (Kotlin, Compose M3, AGP 9 with built-in Kotlin, compileSdk 36, minSdk 23); the engine is GZDoom 4.15 with C++17. NDK r27 quirks are handled in `Application.mk` — read it before changing toolchain settings.
